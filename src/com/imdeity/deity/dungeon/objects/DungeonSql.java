...
public class DungeonSql {
	...
	public static ArrayList<Spawner> getDungeonSpawners(int id) {
		...
		String sql = "SELECT `id`, `name`, `damage_dealt`, `damage_received`, `spawn_world`, `spawn_loc_x`, `spawn_loc_y`, `spawn_loc_z`, `spawn_amount`, `is_boss`, `exp_dropped` FROM " + Deity.data.getDB().tableName("deity_", "dungeon_mobs") + " WHERE dm.`dungeon_id` = ?;";
		...
		if (query != null && query.hasRows()) {
			for (int i = 0; i < query.rowCount(); i++) {
				try {
					...
					for (int ii = 0; i < query2.rowCount(); ii++) {
						short damage = 0;
